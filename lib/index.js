...
class DeploymentBucketPlugin {
  ...
  async waitFor(name, state) {
    const wantExists = state === 'bucketExists'
    const maxAttempts = 30
    const intervalMs = 2000

    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      if ((await this.bucketExists(name)) === wantExists) {
        return true
      }
    ...
    this.serverless.cli.log(`Unable to wait for '${state}' - timed out after ${(maxAttempts * intervalMs) / 1000}s`)
